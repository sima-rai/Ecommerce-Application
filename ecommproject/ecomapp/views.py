...
class AddToCartView(EcomMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        url_pro_id = self.kwargs["pro_id"]
        #get product id
        product_obj = Product.objects.get(id=url_pro_id)

        #check if cart exist
        cart_id = self.request.session.get("cart_id", None)
        print(cart_id, "suruko")
        if cart_id:
            ...
            print(cart.id, "vakkjhr")
            self.request.session["cart_id"] = cart.id
            # cart_id = self.request.session.get(cart_id)
            # print(cart_id, "yoooooooooooooooo")
            product = CartProduct.objects.create(cart=cart, product=product_obj, rate=product_obj.selling_price, quantity=1, subtotal=product_obj.selling_price)
            cart.total += product_obj.selling_price
            cart.save()
...
class EmptyCart(EcomMixin, View):
...
class AdminHomeView(AdminRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["pending"] = Order.objects.filter(order_status="Order Received").order_by("-id")
        return context


class AdminOrderDetail(AdminRequiredMixin, DetailView):
    template_name = "admintemplate/adminorderdetail.html"
    model = Order
    context_object_name = "order"
